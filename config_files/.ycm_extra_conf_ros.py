...
def GetRosIncludePaths():
    ...
    includes = []
    _logger.info("extra: ")
    _logger.info(os.path.expandvars('$ROS_WORKSPACE'))
    _logger.info("ycm_extra_conf")
...
def FlagsForFile(filename):
    ...
    if database:
        ...
        # debug ycm_extra_conf for Youcomplete me, a print statement such as
        # below one would be redirected to /tmp/ycm_temp_<pid>_stdout.log, this
        # file can be read or YcmToggleLogs command can be used in vim to view
        # them, but point of note is that after making any changes to this file
        # one would need to restart vim, YcmRestartServer doesn't produce logs,
        # for whatever reason. So just print the statements, restart vim, and
        # use YcmToggleLogs to see what's happening
        # If ros flags and include paths are not getting included then most
        # probably set(cmake_export_compile_commands 1) is not included in
        # CMakeLists.txt, do that and see if things change for the better
        # print "blue: databse is there"
        ...
        if compilation_info.compiler_working_dir_:
